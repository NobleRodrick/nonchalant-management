...
export const inngest = new Inngest({ id: "nonchalant-management" })
...
// Inngest function to workspace data in database
...
// Inngest funtion to delete workspace data from database
...
// Ingest function to send email on Tak creation
const sendTaskAssignmentEmail = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        await sendEmail({
            ...
            body: `<div style="max-width: 600px;">
                     ...
                     <p style="margin-top: 20px; font-size: 14px; color: #6c757d;">
                        Please make sure to review and complete it before the due date. Don't be nonchalant😅
                     </p>
                   </div>`
        })
        ...
        if (new Date(task.due_date).toLocaleDateString() !== new Date().toDateString()) {
            ...
            await step.run('check-if-task-is-accepted', async () => {
                const task = await prisma.task.findUnique({
                    where: { id: taskId },
                    include: { assignee: true, project: true }
                })
